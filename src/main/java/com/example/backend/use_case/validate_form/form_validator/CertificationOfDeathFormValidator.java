...
public class CertificationOfDeathFormValidator implements FormValidator{
    ...
    private List<FormError> getFormErrors() {
        ...
        if(form.getDateOfDeath() == null){
            FormError error = new FormError("Date of death (mo/day/yr)", "Date of death is either empty or in wrong format");
            errors.add(error);
        }
        ...
        if(form.getDateOfSymptomsOnset() == null){
            FormError error = new FormError("When did symptoms first appear or accident happen (mo/day/yr)", "Date of symptoms onset is either empty or in wrong format");
            errors.add(error);
        }
        if(form.getDateOfSymptomsOnset().isAfter(LocalDate.now())){
            FormError error = new FormError("When did symptoms first appear or accident happen (mo/day/yr)", "Date of symptoms onset cannot be in the future. Please check date format.");
            errors.add(error);
        }
        if(form.getCauseOfDeath() == null){
            FormError error = new FormError("Cause of death", "Cause of death must be one of the 5 options. Please check 1 of them.");
            errors.add(error);
        }
        if(form.getPlaceOfDeath() == null){
            FormError error = new FormError("Place of death (if institution or hospital, give name)", "Place of death cannot be null");
            errors.add(error);
        }
        if(form.getInquestPerformed() == null){
            FormError error = new FormError("Inquest performed", "Inquest performed must be either Yes or No. Please check 1 of them.");
            errors.add(error);
        }
        if(form.getAutopsyPerformed() == null){
            FormError error = new FormError("Autopsy performed", "Autopsy performed must be either Yes or No. Please check 1 of them.");
            errors.add(error);
        }
        ...
        if(form.getAddressOfPhysician() == null){
            FormError error = new FormError("Address (street, city, province, postal code)", "Address of physician cannot be null");
            errors.add(error);
        }
